var systemComponents = {
	...
	'info-tab': {
        ...
        <br><br>
        Mod Made By: {{modInfo.author}}<br><br>
	    ／l、 <br> 
        （ﾟ､ 。 ７<br> 
 　     l、 ~ヽ  l<br>
　      じしf_, )ノ<br>​
        <br>
        Special Thanks: {{modInfo.thanks}}
		<br><br>
        </span>
        <br>
		<br>
        The Modding Tree：<a v-bind:href="'https://github.com/Acamaeda/The-Modding-Tree/blob/master/changelog.md'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >{{TMT_VERSION.tmtNum}}</a> | Made by Acamaeda
        <br>
        The prestige tree made by Jacorb and Aarex
        <br>
        Original: papyrus (Discord)
		<br><br><br><br>
		<div class="link" onclick="showTab('changelog-tab')">The Incremental Tree Changelog</div><br>
        <span v-if="modInfo.discordLink"><a class="link" v-bind:href="modInfo.discordLink" target="_blank">{{modInfo.discordName}}</a><br></span>
        <a class="link" href="https://discord.gg/F3xveHV" target="_blank" v-bind:style="modInfo.discordLink ? {'font-size': '16px'} : {}">The modding tree Discord</a><br>
        <a class="link" href="http://discord.gg/wwQfgPa" target="_blank" v-bind:style="{'font-size': '16px'}">The modding tree Discord</a><br>
		<br><br>
        Time played: {{ formatTime(player.timePlayed) }}<br><br><br><br>
        <h3>Hotkey</h3><br>
    ...
    'options-tab': {
        ...
        <table>
			...
			<td><button class="opt" onclick="hardReset()">HardReset</button></td>
		</tr>
		<tr>
			<td><button class="opt" onclick="exportSave()">Save output to Clipboard</button></td>
			<td><button class="opt" onclick="importSave()">Save Import</button></td>
			<td><button class="opt" onclick="toggleOpt('offlineProd')">Offline: {{ player.offlineProd?"On":"Off" }}</button></td>
		</tr>
		<tr>
			<td><button class="opt" onclick="switchTheme()">Background: {{ getThemeName() }}</button></td>
			<td><button class="opt" onclick="toggleOpt('hqTree')">High quality node: {{ player.hqTree?"On":"Off" }}</button></td>
		</tr>
		<tr>
			<td><button class="opt" onclick="toggleOpt('hideChallenges')">Complete Challenges: {{ player.hideChallenges?"Hide":"Show" }}</button></td>
			<td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">Half Show Mode: {{ player.forceOneTab?"Always":"Auto" }}</button></td>
			</tr> 
        </table>`
    },
